...
use crate::{note::Note, json_note::JsonNote};

pub fn load_notes_from_json<P: AsRef<Path> + std::fmt::Debug>(path: P) -> Result<HashMap<Uuid, Note>, String> {
        ...
        Ok(x) => {
            for json_note in x {
                let uuid = match Uuid::parse_str(&json_note.uuid) {
                    Ok(x) => x,
                    Err(err) => return Err(format!("failed to connvert {} into a UUID with {err:?}", json_note.uuid)),
                };
